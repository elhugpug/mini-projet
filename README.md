...
Calcul d'une partie du morpho-math :
```
dilatation <- focal(grapes, w=matrix(2, ncol=3, nrow=3),  fun=max)
fermeture <- focal(dilatation, w=matrix(1, ncol=3, nrow=3), fun=main)
```
...
La catégorie *olives* est la plus simple à traiter. Comme on peut le voir sur le graphique plus haut, l'olivier possède un tracé caractéristique : une très forte baisse du NDVI entre la 6ème et la 7ème date, ce que l'on ne retrouve pas ailleurs. Le blé et la parcelle d'arbre fruitier sont gérés de la même manière (bien que leurs caractéristiques soient moins évidente à mettre en avant). Dans les trois cas, les parcelles sont bien classé et la vérification via google map et plutôt correct. 
